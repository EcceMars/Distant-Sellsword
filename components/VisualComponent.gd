...
func _init(
	...
	anim_key:String = ""  ## Animation key from AnimationRegistry
	) -> void:
...
func _create_sprite(REG:REGISTRY, anim_key:String) -> void:
	match sprite_type:
		SpriteType.STATIC:
			...
			if anim_key:
				pass
				#var texture:Texture2D = AnimationRegistry.get_frames(anim_key) as Texture2D
				#if texture:
		...
		SpriteType.ANIMATED:
			...
			if anim_key:
				var frames:SpriteFrames = AnimationRegistry.get_frames(anim_key)
				if frames:
					sprite.sprite_frames = frames
				else:
					push_warning("[VisualComponent] Animation '%s' not found, using debug visual" % anim_key)
					_default(REG)
					return
			sprite.centered = true
